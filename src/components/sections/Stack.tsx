  ...
  return (
    <section className="relative w-full py-[3rem] lg:py-[clamp(6rem,10vh,10rem)] overflow-hidden bg-[#050511] flex flex-col items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-[1600px] flex flex-col items-center gap-[1.5rem] lg:gap-[clamp(2rem,5vh,4rem)]">
        ...
        {/* Marquee Section */}
        <div className="w-full flex flex-col gap-2 sm:gap-6 mt-4 sm:mt-8">
          {/* Row 1: Left Scroll */}
...
function Marquee({
  ...
  return (
    // FIX: Added py-4 sm:py-6 to give the cards vertical room to scale without clipping
    <div className="relative flex overflow-hidden py-4 sm:py-6 group w-full select-none">
      ...
      {/* FIX: Added items-center so cards stay perfectly in the middle of the padded track */}
      <div
        className={`marquee-container flex items-center gap-4 sm:gap-8 min-w-full ${direction === "left" ? "animate-scroll-left" : "animate-scroll-right"}`}
...
function GlassCard({
  ...
  tech: { name: string; icon: React.ReactNode; color: string };
}) {
  return (
