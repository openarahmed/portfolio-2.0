...
function AIChatModal({
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "model", text: "Connection error." },
      ]);
    } finally {
      setLoading(false);
    }
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
          {messages.length === 0 && (
            <div className="text-center text-slate-400 text-sm mt-10 space-y-2">
              <Cpu
                size={40}
                className="mx-auto text-violet-400 mb-2 opacity-80"
              />
          ...
          {messages.map((m, i) => (
            ...
            >
              <div
                className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 border border-white/10 ${m.role === "user" ? "bg-violet-600" : "bg-slate-800"}`}
              >
                {m.role === "user" ? (
...
const WhatsAppWidget = () => {
  ...
  return (
    <>
      <div
        className="fixed z-[100] flex flex-col items-center gap-4 
        bottom-24 right-4 
      ...
      >
        ...
        >
          ...
          {isChatOpen ? (
            ...
            <Cpu
              strokeWidth={1.5}
              className="w-full h-full text-purple-400 drop-shadow-[0_0_10px_rgba(192,132,252,0.8)]"
            />
          )}
